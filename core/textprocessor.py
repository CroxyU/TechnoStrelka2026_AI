...
if __name__ == "__main__":
    chunks = process_book(Path(__file__).parent /"test.txt", "test_book")
    for chunk in chunks:
        print(f"Book: {chunk['book_id']}, Chapter: {chunk['chapter']}, Index: {chunk['chunk_index']}")
        print(chunk['text'])  # печатаем начало
        print("-" * 40)
